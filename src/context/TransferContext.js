...
export function TransferContextProvider({ children }) {
  ...
  useEffect(() => {
    ...
    async function populateTransfers() {
      ...
      if (mainAccount) {
        transferItems = await MothershipContract.getPendingTransfersOfBuyer(
          mainAccount
        );

        for (let address of transferItems) {
          const ProvenanceTransferContract = new ethers.Contract(
            address,
            Provenance.abi,
            signer
          );
          const ProvenanceTransferProps =
            await ProvenanceTransferContract.instrument();
          const index = ProvenanceTransferContract.ownerCount();
          const ProvenanceOwnerInfo =
            await ProvenanceTransferContract.ownerProvenance(index);

          transferArray.push({
            ProvenanceContract: ProvenanceTransferContract,
            ProvenanceProps: ProvenanceTransferProps,

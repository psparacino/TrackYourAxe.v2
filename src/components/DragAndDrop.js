    ...
    */
    ...
    const uploadFiles = async() => {
        ...
        //uploadRef.current.innerHTML = 'File(s) Uploading...';
            ...
            Promise.all(validFiles.map((validFile) => {
    ...
    const PhotoUploadMessage =() => {
        let photoMsg;
        if (photoLimit === 1) {
            const msg = formDataImport.verificationphotohash.length > 0 ? 'Verication Photo Successfully Uploaded. Upload Another Photo to Change' : 'Upload Your Verification Photo to Mint Token';
            photoMsg = msg;
        }
                        ...
                        validFiles.map((data, i) => 
                            <div className="file-status-bar" key={i}>
                                <div onClick={!data.invalid ? () => openImageModal(data) : () => removeFile(data.name)}>
                                    ...
                                    {itemPhotoLimitError && <span className='file-error-message'>({itemPhotoLimitError})</span>}
                                </div>
                                <div className="file-remove" onClick={() => removeFile(data.name)}>X</div>
                            </div>
                        )
                    }
                    {photoLimitMessage && <span className='file-error-message'>({photoLimitMessage})</span>}
                </div>
            </div>

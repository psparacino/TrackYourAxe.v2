...
const PhotoPreviews = ({
  ...
  // let itemPhotoArray = [];
  let verificationHashURL =
    "https://gateway.pinata.cloud/ipfs/" + formData.verificationphotohash;
  ...
  useEffect(async () => {
    ...
    async function loadItemHashes() {
      ...
      for (let i = 0; i < (await formData.instrumentphotohashes).length; i++) {
        let itemPhotosHashURL =
          ipfsGetterRootURL + formData.instrumentphotohashes[i];
      ...
      <div className="previewContainer">
        ...
        {photoLimit == 1 ? (
          <div className="previewPhoto">
            ...
            {formData.verificationphotohash.length == 0 ? null : (
              <img
                src={verificationHashURL}
                alt="verification photo not yet uploaded"
              />
            )}
          </div>
        ) : (

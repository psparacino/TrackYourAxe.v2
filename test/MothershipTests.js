...
describe("Mothership Tests", function () {
  ...
  it("sale of from one owner to another", async function () {
    ...
    const ProvenanceContractTestSecondSigner =
      ProvenanceContractTest.connect(addr2);

    await ProvenanceContractTestSecondSigner.connect(addr2).claimOwnership(
      provenanceOwner,
      ethers.utils.formatBytes32String("verificationPhoto2"),
      ethers.utils.formatBytes32String("12/31/1999")
    );
  ...
  it("get all Provenances from All Owners", async function () {
    ...
    const create10Provenances = async () => {
      ...
      await MothershipContract.connect(addr1)
        .createBatchProvenances(
          type1,
          serial1,
          brand1,
          model1,

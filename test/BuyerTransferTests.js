...
describe("Buyer Initiated Transfer Tests", function () {
  ...
  it("owner cannot make offer on own Provenance", async function () {
    await expect(
      ProvenanceContractTest.connect(addr1).makeOffer({ value: 1000000 })
    ).to.be.revertedWith(
      "VM Exception while processing transaction: reverted with reason string 'You cannot purchase your own Provenance'"
    );
  });
  ...
  it("buyer cannot make offer lower than existing offer", async function () {
    await ProvenanceContractTest.connect(addr2).makeOffer({ value: 1000000 });
    await expect(
      ProvenanceContractTest.connect(addr3).makeOffer({ value: 100 })
    ).to.be.revertedWith(
      "VM Exception while processing transaction: reverted with reason string 'Offer must greater than current standing offer'"
    );
  });
  ...
  it("owner or buyer can cancel existing offer", async function () {
    ...
    await ProvenanceContractTest.connect(addr2).makeOffer({
      value: 100000000000,
    });
    const balanceInWeiBeforeCancel = await provider.getBalance(
      ProvenanceContractTest.address
    );
    expect(balanceInWeiBeforeCancel).to.equal(100000000000);
    await ProvenanceContractTest.cancelOffer();
    const balanceInWeiAfterCancel = await provider.getBalance(
      ProvenanceContractTest.address
    );
  ...
  it("owner accepts offer", async function () {
    ...
    expect((await ProvenanceContractTest.currentOffer()).buyer).to.equal(
      addr2.address
    );
    ...
    expect(
      (await ProvenanceContractTest.ownerProvenance(ownerNumber)).ownerAddress
    ).to.equal(addr2.address);
    expect(
      (await ProvenanceContractTest.ownerProvenance(ownerNumber - 1))
        .ownerAddress

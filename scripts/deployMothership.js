...
const token =
  require("../src/deployedContractAddresses/instrumenttokenaddress.json").address;
...
async function main() {
  ...
  fs.writeFileSync(
    "./src/deployedContractAddresses/mothershipaddress.json",
    `{ "address": "${mothership.address}" }`,
    (err) => {
      if (err) console.log(err);
      else {
        console.log("Mothership address written successfully\n");
      }
    }
  );
}
